function varargout = NodalizeDesignDomain(varargin)
	if nargin<2 || nargin>3, error('Wrong input for nodalization!'); end
	if 2==length(varargin{1}) & 2==size(varargin{2},2)
		if 2~=nargout, error('Wrong output for nodalization!'); end
		numSeed = varargin{1};
		nx = numSeed(1); ny = numSeed(2);
		dd = varargin{2}; %%design domain
		...
		if 3==nargin & strcmp(varargin{3}, 'inGrid')
			varargout{1} = reshape(varargout{1}, ny+1, nx+1);
			varargout{2} = reshape(varargout{2}, ny+1, nx+1);
		end		
	elseif 3==length(varargin{1}) & 3==size(varargin{2},2)
		if 3~=nargout, error('Wrong output for nodalization!'); end
		numSeed = varargin{1};
		...
		error('Wrong input for nodalization!');
	end
end
