function Interaction_PickBySelectionBox2D(axHandle, cP1, cP2)
	global surfMesh_;
	...
	if numNewlyPickedNodes>0
		...
		if isempty(hdPickedNode_)
			hdPickedNode_ = plot(axHandle, surfMesh_(1).nodeCoords(nodesWithinSelectionBox,1), ...
				surfMesh_(1).nodeCoords(nodesWithinSelectionBox,2), 'xr', 'LineWidth', 2, 'MarkerSize', 6);
		else
			hdPickedNode_(end+1) = plot(axHandle, surfMesh_(1).nodeCoords(nodesWithinSelectionBox,1), ...
				surfMesh_(1).nodeCoords(nodesWithinSelectionBox,2), 'xr', 'LineWidth', 2, 'MarkerSize', 6);		
		end
