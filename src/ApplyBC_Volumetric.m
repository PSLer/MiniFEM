classdef ApplyBC_Volumetric < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function ImportSolidmeshmshmeshvtkMenuSelected(app, event)
            ...
            [fileName, dataPath] = uigetfile({'*.msh'; '*.mesh'; '*.vtk'}, 'Select a Volumetric Mesh File to Open');
            if isnumeric(fileName) || isnumeric(dataPath), return; end
            [~,~,fileExtension] = fileparts(fileName);
            if ~(strcmp(fileExtension, '.msh') || strcmp(fileExtension, '.mesh') || strcmp(fileExtension, '.vtk'))
                warning('Un-supported Mesh Format!');
                return;
            end
            inputVoxelfileName = strcat(dataPath,fileName);
            IO_ImportVolumetricMesh(inputVoxelfileName);
    ...
    methods (Access = public)
        ...
        % Construct app
        function app = ApplyBC_Volumetric(varargin)
